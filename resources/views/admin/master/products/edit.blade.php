    ...
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow p-6">
        ...
        <div class="mt-8" x-data="{ 
                ...
                <form action="{{ route('admin.master.products.discount.store', $product->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <select name="discount_type" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 text-sm focus:border-blue-500 focus:outline-none" required>
                        ...
                        <div>
                            ...
                            <input type="number" name="discount_value" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 text-sm focus:border-blue-500 focus:outline-none" required min="0">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 uppercase">Mulai</label>
                            <input type="date" name="start_date" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 text-sm focus:border-blue-500 focus:outline-none" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 uppercase">Berakhir</label>
                            <input type="date" name="end_date" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 text-sm focus:border-blue-500 focus:outline-none" required>
                        </div>
                    </div>
                    <div class="mt-4">
                        ...
                        @forelse($product->discounts()->latest()->get() as $discount)
                            <tr>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-xs font-medium flex gap-2">
                                    ...
                                    <form action="{{ route('admin.master.products.discount.toggle', $discount->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="{{ $discount->is_active ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900' }}">
                                            {{ $discount->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                        </button>
                                    </form> |
                                    <form action="{{ route('admin.master.products.discount.destroy', $discount->id) }}" method="POST" onsubmit="return confirm('Hapus riwayat diskon ini?')">
                                        @csrf
                                        @method('DELETE')
            ...
            <div x-show="editModalMode" 
                 ...
                 x-cloak>
                <div @click.away="editModalMode = false" 
                    ...
                    <form :action="editDiscount.url" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 uppercase">Tipe</label>
                                <select name="discount_type" x-model="editDiscount.type" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-blue-500 focus:outline-none" required>
                                    <option value="PERCENTAGE">Persentase (%)</option>
                                    <option value="FIXED">Nominal (Rp)</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 uppercase">Nilai</label>
                                <input type="number" name="discount_value" x-model="editDiscount.value" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-blue-500 focus:outline-none" required min="0">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 uppercase">Mulai</label>
                                    <input type="date" name="start_date" x-model="editDiscount.start" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-blue-500 focus:outline-none" required>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 uppercase">Berakhir</label>
                                    <input type="date" name="end_date" x-model="editDiscount.end" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-blue-500 focus:outline-none" required>
                                </div>
                            </div>
                        </div>
                        <div class="mt-6 flex justify-end gap-3">
                            <button type="button" @click="editModalMode = false" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                                Batal
                            </button>
